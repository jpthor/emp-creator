...
export const getSwapOutput = (inputAmount, pool, toVether) => {
  ...
  const X = toVether ? bn(pool.balance) : bn(pool.depth) // input is token if toVether
  const Y = toVether ? bn(pool.depth) : bn(pool.balance) // output is perlin if toVether
...
export const getSwapFee = (inputAmount, pool, toVether) => {
  // formula: (x * x * Y) / (x + X) ^ 2
  const x = bn(inputAmount)
  const X = toVether ? bn(bn(pool.balance)) : bn(pool.depth) // input is token if toVether
  const Y = toVether ? bn(pool.depth) : bn(bn(pool.balance)) // output is perlin if toVether
...
export const getPoolShare = (unitData, pool) => {
  // formula: (perlin * part) / total; (token * part) / total
  ...
  const perlin = V.times(units).div(total)
  const stakeData = {
    token: token,
    perlin: perlin,
  }
  // console.log((stakeData.token).toFixed(0), (stakeData.perlin).toFixed(0))
  return stakeData
}
